  ...
  async function cargarReportes() {
    try {
      ...
      const { data, error: reportesError } = await supabase
        ...
        .select('id_reporte, nombre_reporte, region, comuna, timestamp_creado, timestamp_publicacion, categoria_general, tipo_riesgo_principal, autor_reporte, codigo_accesibilidad, tipologia_especifica')
    ...
    return (
      ...
      >
        <div className="flex items-center gap-3 text-sm" style={{ color: 'var(--muted)' }}>
          ...
          <span>Cargando catálogo de publicados...</span>
        </div>
      </div>
    )
  }
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8 pb-6 border-b" style={{ borderColor: 'var(--border)' }}>
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="w-2 h-2 rounded-full" style={{ backgroundColor: 'var(--musgo)' }} />
              <span className="text-[11px] font-medium tracking-widest uppercase" style={{ color: 'var(--muted)' }}>
                Catálogo Activo · Estado Verde
              </span>
            </div>
            ...
            <p className="text-sm mt-1" style={{ color: 'var(--muted)' }}>
              {reportes.length} ficha{reportes.length !== 1 ? 's' : ''} validada{reportes.length !== 1 ? 's' : ''} y disponible{reportes.length !== 1 ? 's' : ''} en el sistema
            </p>
          </div>
            ...
            return (
              ...
              >
                ...
                <span
                  ...
                  style={{
                    backgroundColor: isActive ? 'rgba(78,138,96,0.15)' : 'rgba(255,255,255,0.04)',
                    color: isActive ? 'var(--musgo)' : 'var(--faint)',
                  }}
                >
                  {count}
                </span>
        ...
        {reportesFiltrados.length === 0 ? (
          ...
          >
            <p className="text-sm" style={{ color: 'var(--muted)' }}>
              No hay reportes publicados en esta categoría
            </p>
          </div>
        ) : (
          <div className="space-y-3.5">
            {reportesFiltrados.map((reporte) => {
              ...
              const codigo = reporte.codigo_accesibilidad || 'B'
                  ...
                  <div className="flex flex-row gap-4 sm:gap-5 items-start">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-between self-stretch">
                        ...
                        <div className="flex items-center justify-between gap-2 mb-1">
                          <div className="flex items-center gap-1.5">
                            <span
                              ...
                              style={{
                                backgroundColor: riesgo ? 'rgba(168,80,64,0.14)' : 'rgba(78,138,96,0.14)',
                                color: riesgo ? 'var(--ladrillo)' : 'var(--musgo)',
                              }}
                            >
                              {riesgo ? 'Riesgo Activo' : 'Publicado'}
                            </span>
                            ...
                            <span
                              ...
                              style={{
                                backgroundColor: 'var(--surface-2)',
                                color: codigo === 'A' ? 'var(--musgo)' : codigo === 'B' ? 'var(--cobre)' : 'var(--muted)',
                                border: '1px solid var(--border-m)',
                              }}
                            >
                              Código {codigo}
                      ...
                      <div className="flex items-center justify-between pt-2 border-t text-xs mt-1" style={{ borderColor: 'var(--border)' }}>
                        ...
                        <div className="flex items-center gap-1 text-xs font-medium flex-shrink-0" style={{ color: 'var(--musgo)' }}>
                          <span>Ver ficha</span>
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
