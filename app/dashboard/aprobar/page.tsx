...
interface Reporte {
  ...
  timestamp_revision: string | null
  ...
  async function cargarReportes() {
    try {
      ...
      const { data, error: reportesError } = await supabase
        ...
        .select('id_reporte, nombre_reporte, region, comuna, timestamp_creado, timestamp_revision, estado_validacion, categoria_general, tipo_riesgo_principal, autor_reporte, codigo_accesibilidad, tipologia_especifica')
      ...
      setError('Error al cargar reportes listos para publicación')
    } finally {
      setLoading(false)
    }
  }
    ...
    return (
      ...
      >
        <div className="flex items-center gap-3 text-sm" style={{ color: 'var(--muted)' }}>
          ...
          <span>Cargando reportes para aprobación...</span>
        </div>
      </div>
    )
  }
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8 pb-6 border-b" style={{ borderColor: 'var(--border)' }}>
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="w-2 h-2 rounded-full" style={{ backgroundColor: 'var(--cobre)' }} />
              <span className="text-[11px] font-medium tracking-widest uppercase" style={{ color: 'var(--muted)' }}>
                Bandeja Editorial · Estado Amarillo
              </span>
            </div>
            ...
            <p className="text-sm mt-1" style={{ color: 'var(--muted)' }}>
              {reportes.length} reporte{reportes.length !== 1 ? 's' : ''} revisado{reportes.length !== 1 ? 's' : ''} listo{reportes.length !== 1 ? 's' : ''} para catalogación final
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 self-start sm:self-auto">
            <Link
              ...
              className="inline-flex items-center gap-2 text-xs font-medium px-3.5 py-2 rounded-lg transition"
              style={{
                backgroundColor: 'rgba(78,138,96,0.12)',
                color: 'var(--musgo)',
            ...
            <Link
              href="/perfil"
              className="inline-flex items-center gap-1.5 text-xs font-medium px-3.5 py-2 rounded-lg transition"
              style={{
        ...
        {reportesFiltrados.length === 0 ? (
          ...
          >
            <p className="text-sm" style={{ color: 'var(--muted)' }}>
              No hay reportes en revisión técnica en esta categoría
            </p>
          </div>
        ) : (
                  ...
                  <div className="flex flex-row gap-4 sm:gap-5 items-start">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-between self-stretch">
                      <div>
                        ...
                        <div className="flex items-center justify-between gap-2 mb-1">
                          <span
                            ...
                            style={{
                              backgroundColor: riesgo ? 'rgba(168,80,64,0.14)' : 'rgba(194,120,64,0.14)',
                              color: riesgo ? 'var(--ladrillo)' : 'var(--cobre)',
                            }}
                          >
                            {riesgo ? 'Riesgo Validado' : 'Revisión Completada'}
                          </span>
                      ...
                      <div className="flex items-center justify-between pt-2 border-t text-xs mt-1" style={{ borderColor: 'var(--border)' }}>
                        ...
                        <div className="flex items-center gap-1 text-xs font-medium flex-shrink-0" style={{ color: 'var(--cobre)' }}>
