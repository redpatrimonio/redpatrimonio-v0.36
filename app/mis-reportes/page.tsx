  ...
  async function cargarReportes() {
    ...
    try {
      ...
      const { data, error: reportesError } = await supabase
        ...
        .select('id_reporte, nombre_reporte, region, comuna, timestamp_creado, estado_validacion, categoria_general, tipo_riesgo_principal, codigo_accesibilidad, tipologia_especifica')
    ...
    switch (estado) {
      case 'verde':
        return {
          label: 'Publicado',
          color: 'var(--musgo)',
          bg: 'rgba(78,138,96,0.14)',
          desc: 'Validado y visible en el mapa'
        }
      case 'amarillo':
        return {
          label: 'En Revisión',
          color: 'var(--cobre)',
          bg: 'rgba(194,120,64,0.14)',
          desc: 'En catalogación técnica'
        }
      case 'rojo':
      default:
        return {
          label: 'Pendiente',
          color: 'var(--ladrillo)',
          bg: 'rgba(168,80,64,0.14)',
          desc: 'En espera de evaluación'
        }
    }
  }
      ...
      <div className="max-w-4xl mx-auto">
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[11px] font-medium tracking-widest uppercase" style={{ color: 'var(--muted)' }}>
                Historial Ciudadano · Seguimiento
              </span>
            </div>
            ...
            <p className="text-sm mt-1" style={{ color: 'var(--muted)' }}>
              {reportes.length} reporte{reportes.length !== 1 ? 's' : ''} enviado{reportes.length !== 1 ? 's' : ''} a la plataforma
            </p>
          </div>
        ...
        {reportesFiltrados.length === 0 ? (
          ...
          >
            ...
            <Link
              href="/reportar-inicio"
              className="inline-flex items-center gap-2 text-xs font-medium px-4 py-2 rounded-lg transition"
              style={{
                backgroundColor: 'var(--btn-light)',
                color: '#111110',
              }}
          ...
          <div className="space-y-3.5">
            {reportesFiltrados.map((reporte) => {
              ...
              const estado = getEstadoConfig(reporte.estado_validacion)
                ...
                >
                  <div className="flex flex-row gap-4 sm:gap-5 items-start">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-between self-stretch">
                      ...
                      {/* Footer: Estado descriptivo y acción */}
